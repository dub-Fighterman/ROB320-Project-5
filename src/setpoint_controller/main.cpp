...
int main(int argc, char **argv) {
    rix::util::ArgumentParser parser("setpoint_controller", "Publishes JS messages");
    parser.add<std::string>("file", "Robot description file");
    parser.add<std::string>("end_effector", "End effector");
    ...
    if (!parser.get<std::string>("end_effector", end_effector)) {
        rix::util::Log::error << "Failed to get file argument" << std::endl;
        return 1;
    }
    ...
    auto traj_srv = std::make_shared<SetpointController>(rixhub_endpoint, robot, rate, end_effector);
